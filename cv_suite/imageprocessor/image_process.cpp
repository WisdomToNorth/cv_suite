...
#include "image/image_algorithm.h"
...
namespace image {
	...
	double ImageProcessTest::_Correlation1D(const cv::Mat& input1, const cv::Mat& input2) {
		...
		cv::reduce(input1, row_mean1, 1, CV_REDUCE_AVG, CV_MAKE_TYPE(CV_64F, cn));
		cv::reduce(input2, row_mean2, 1, CV_REDUCE_AVG, CV_MAKE_TYPE(CV_64F, cn));
